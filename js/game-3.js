...
function loadImg(src) {
  ...
  return new Promise(res => {
    ...
    img.src = src + '?v=' + Date.now(); // منع الكاش القديم
  });
}
...
function updateLoadBar() {
  ...
  if (txt) txt.textContent = `Loading... ${pct}% (${loadedCount}/${totalToLoad})`;
}
...
async function loadCriticalAssets() {
  ...
  const timeout = new Promise(res => setTimeout(res, 30000));
...
const GOOGLE_CLIENT_ID = '440239520472-n4hfnn0bp03cq7rj0qpge159ie5pas29.apps.googleusercontent.com';
...
window.addEventListener('load', async () => {
  ...
  ldProgress(25, 'Loading assets... 0%');
  // شريط التحميل يتحدث تلقائياً عبر updateLoadBar
