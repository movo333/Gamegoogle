...
function showScreen(id) {
  const mains = ['loadingScreen','splash','worldSelect','levelSelect','gameScreen'];
    ...
    setTimeout(() => {
      try {
        ...
        if (isWebView) {
          ...
          if (fb) {
            fb.textContent = '🌐 Login via Browser';
            fb.style.display = 'block';
            fb.onclick = () => {
              if (window.AndroidBridge && window.AndroidBridge.openInBrowser) {
                window.AndroidBridge.openInBrowser('https://movo333.github.io/Pio_inc/');
              } else {
                window.open('https://movo333.github.io/Pio_inc/', '_blank');
              }
            };
          }
          return;
